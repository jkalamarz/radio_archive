...
TARGET=$HOME/public_html/nowyswiat/`echo $PREFIX | cut -c 1-10`
...
echo rm -r $HOME/public_html/nowyswiat/$WEEKOLD
[ -d $HOME/public_html/nowyswiat/$WEEKOLD ] && rm -r $HOME/public_html/nowyswiat/$WEEKOLD

[ "" == "$2" ] || exit

LAST=`find ~/tmp/nowyswiat -iname '*.mp3' -mmin -1 -type f`
rm -f $HOME/public_html/nowyswiat/NOW.mp3
ln -s "$LAST" $HOME/public_html/nowyswiat/NOW.mp3
